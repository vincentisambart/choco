...
extern "C" {
    ...
    fn CFRetain(cf: RawPtr) -> Option<RawPtr>;
}
...
impl RawPtr {
    pub unsafe fn retain<T: Type>(&self) -> OwnedPtr<T> {
        let retained = unsafe {
            match T::KIND {
...
pub trait PtrHolder<T: Type>: AsRaw {}

pub struct OwnedPtr<T: Type> {
    raw: RawPtr,
...
where
    T: Type,
{
    pub(crate) unsafe fn from_owned_raw_unchecked(raw: RawPtr) -> Self {
        Self {
            raw,
            _marker: PhantomData,
        }
    ...
    T: Type,
{
    fn drop(&mut self) {
        unsafe {
            match T::KIND {
                TypeKind::ObjC => objc_release(self.raw),
                TypeKind::CF => CFRelease(self.raw),
            }
        }
    }
}

impl<T> PtrHolder<T> for OwnedPtr<T> where T: Type {}
